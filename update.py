...
# ── Step 4: Merge MongoDB vars into config.env AFTER git reset ───────────────
# Strategy (priority high → low):
#   1. MongoDB vars  — always written (saved via Colab config cell or /botsettings)
#   2. SSLeech repo's config.env — git reset restores it; extra vars here are
#      preserved so user doesn't have to re-enter them via /botsettings.
#      MongoDB vars silently override any duplicate keys from the file.
#
# Result: every restart = MongoDB vars + all extra vars from SSLeech config.env.
# bot/__init__.py then reads the merged file via load_dotenv("config.env").
...
# 1. Start with SSLeech repo's config.env (restored by git reset)
_merged: dict[str, str] = {}
if path.exists("config.env"):
    _merged = _parse_env_file("config.env")
    _LOGGER.info(
        f"SSLeech config.env: loaded {len(_merged)} existing var(s) as base."
    )

# 2. Overlay MongoDB vars on top (MongoDB wins on duplicate keys)
_mongo_written = 0
if db_doc:
    for key, value in db_doc.items():
...
# 3. Write merged result back to config.env
if _merged:
    _out_lines = [
        "# Auto-generated by update.py — SSLeech repo vars + MongoDB overrides.",
        "# Do not edit manually; changes here are overwritten on every restart.",
        "",
    ]
    for _k, _v in _merged.items():
        _v_esc = _v.replace('"', '\\"')
        _out_lines.append(f'{_k}="{_v_esc}"')
    try:
        with open("config.env", "w") as f:
            f.write("\n".join(_out_lines) + "\n")
        _LOGGER.info(
            f"config.env written: {len(_merged)} total var(s) "
            f"({_mongo_written} from MongoDB, rest from SSLeech repo)."
        )
    except Exception as e:
